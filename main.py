...
def main():
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        #print('successfully loaded the checkpoint from moco+DeeplabV3+! ')
        #
        #state_dict = torch.load("./pre_models/resnet50_mocov2_800es.pth")
        ##print(state_dict)
        ##exit()
        #state_dict = state_dict['state_dict'] 
        #state_dict =  {'backbone.'+k.split('module.encoder_q.')[1]: v for k, v in state_dict.items()if(k in state_dict and 'module.encoder_q' in k)}
        #state_dict.popitem()
        #state_dict.popitem()
        #state_dict.popitem()
        #state_dict.popitem()
        ...
        #model_dict.update(state_dict)
        #model.load_state_dict(model_dict)
        #print("successfully loaded the offline model ResNet50 from mocov2(resnet50_mocov2_800es.pth)!")     
        ...
        #model_dict = model.state_dict()
        #state_dict = torch.load('./pre_models/resnet50_obow_2048_aut_200es_test_best.pth')
        ...
        #model_dict.update(state_dict)
        #model.load_state_dict(model_dict)
        #print('successfully loaded the resnet50_obow_2048_aut_200es_test_best.pth')
        if opts.continue_training:
            optimizer.load_state_dict(checkpoint["optimizer_state"])#.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
